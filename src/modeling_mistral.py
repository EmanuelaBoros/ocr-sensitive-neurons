...
from modeling_llama import select_random_neurons
...
class MistralMLP(nn.Module):
    ...
    def forward(self, x, dampen_neurons=None, decrease=0.1):
        ...
        return self.down_proj(self.act_fn(self.gate_proj(x)) * up_output)
...
class MistralForTokenClassification(MistralPreTrainedModel):
    def __init__(self, config, target_indices=[], num_neurons=1000, decrease=0.1):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.model = MistralModel(config, target_indices, num_neurons, decrease)
    ...
    def forward(
        ...
        # Ensure that labels are on the same device as the model
        device = next(self.model.parameters()).device

        # # Assuming `logits` and `labels` are defined elsewhere in your code
        # logits = self.model(input_ids).logits  # Example forward pass, adjust as necessary
        # labels = labels.to(device)  # Move labels to the same device as logits

        sequence_output = outputs[0]

        sequence_output = self.dropout(sequence_output)
        logits = self.classifier(sequence_output).to(device)
