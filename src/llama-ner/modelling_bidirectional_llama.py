...
# from transformers import BertModel, LlamaModel, LlamaConfig
# from transformers.modeling_outputs import TokenClassifierOutput
# from typing import Optional, Union, Tuple
#
#
# class LlamaBertForTokenClassification(nn.Module):
#     def __init__(self, config, num_labels, use_bert=True):
#         super(LlamaBertForTokenClassification, self).__init__()
#         self.config = config
...
#
#         # Initialize BERT model if use_bert is True
#         if self.use_bert:
...
#     def forward(
#         self,
#         input_ids_bert: Optional[torch.LongTensor] = None,
#         attention_mask_bert: Optional[torch.Tensor] = None,
#         input_ids_llama: Optional[torch.LongTensor] = None,
#         attention_mask_llama: Optional[torch.Tensor] = None,
#         labels: Optional[torch.LongTensor] = None,
#         return_dict: Optional[bool] = None,
...
#         )
#
#         # Process inputs through BERT if use_bert is True
#         if self.use_bert:
...
#                 return_dict=return_dict,
#             )
#             bert_sequence_output = self.bert_dropout(bert_outputs[0])
#         else:
#             bert_sequence_output = None
#
#         # Process inputs through LLaMA model
#         llama_outputs = self.llama_model(
#             input_ids=input_ids_llama,
#             attention_mask=attention_mask_llama,
#             return_dict=return_dict,
#         )
#         llama_sequence_output = llama_outputs.last_hidden_state
#
#         # If using BERT, concatenate its output with LLaMA's output
#         if self.use_bert and bert_sequence_output is not None:
#             sequence_output = torch.cat(
#                 (llama_sequence_output, bert_sequence_output), dim=-1
#             )
#         else:
#             sequence_output = llama_sequence_output
#
#         sequence_output = self.dropout(sequence_output)
#         logits = self.classifier(sequence_output)
#
...
class LlamaBertForTokenClassification(LlamaPreTrainedModel):
    def __init__(self, config, use_bert=False):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.use_bert = use_bert
        ...
        if self.use_bert:
            ...
            self.bert = BertModel.from_pretrained(
                "bert-base-multilingual-cased", config=bert_config
            )
            self.bert_dropout = nn.Dropout(0.1)
    ...
    def forward(
        self,
        input_ids_bert: torch.LongTensor = None,
        attention_mask_bert: Optional[torch.Tensor] = None,
        input_ids_llama: torch.LongTensor = None,
        attention_mask_llama: Optional[torch.Tensor] = None,
        position_ids: Optional[torch.LongTensor] = None,
        past_key_values: Optional[List[torch.FloatTensor]] = None,
        inputs_embeds: Optional[torch.FloatTensor] = None,
        ...
        # Use BERT encoder if use_bert is True
        if self.use_bert:
            ...
            encoder_sequence_output = self.bert_dropout(encoder_outputs[0])
        else:
        ...
        # If using BERT, concatenate its output with LLaMA's output
        if self.use_bert and encoder_sequence_output is not None:
            sequence_output = torch.cat(
                (sequence_output, encoder_sequence_output), dim=-1
            )
